package com.plgrim.ncp.framework.exception.api;
...
public class RestfulExceptionMapper implements ExceptionMapper<Exception> {
    ...
    public Response toResponse(Exception exception) {
        ...
        String actionId = MDC.get(ExecutionContext.REQUEST_ID);        
        Logger logger = log;
        ...
        if (className.startsWith("org.springframework.security")) {
            ...
            MDC.put("restfulError", actionId);
            logger.error("ERROR Occured during execute rest api: {}", exception.getMessage(), exception);
        }
        
        return createResponse(status, ApiResponse.ERROR, "["+actionId+"] Server Error:"+exception.getMessage());
    }
