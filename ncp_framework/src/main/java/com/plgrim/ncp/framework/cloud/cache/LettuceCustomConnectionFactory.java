 ...
 */
public class LettuceCustomConnectionFactory implements InitializingBean, DisposableBean, RedisConnectionFactory {
	 ...
	 */
	public LettuceCustomConnectionFactory() {}

	/**
	 * Constructs a new <code>LettuceConnectionFactory</code> instance with default settings.
	 */
	public LettuceCustomConnectionFactory(String host, int port) {
		this.hostName = host;
		this.port = port;
	}
	 ...
	 * @since 1.6
	 */
	public LettuceCustomConnectionFactory(RedisSentinelConfiguration sentinelConfiguration) {
	 ...
	 * @since 1.7
	 */
	public LettuceCustomConnectionFactory(RedisClusterConfiguration clusterConfig) {
		this.clusterConfiguration = clusterConfig;
	}

	public LettuceCustomConnectionFactory(LettucePool pool) {
		this.pool = pool;
	}
	...
	public RedisClusterConnection getClusterConnection() {
		...
		return new LettuceCustomClusterConnection((RedisClusterClient) client, getTimeout(), clusterCommandExecutor);
	}
	...
	protected StatefulRedisConnection<byte[], byte[]> createLettuceConnector() {
		try {
			...
			if (client instanceof RedisClient) {
				connection = ((RedisClient) client).connect(ByteArrayCodec.INSTANCE);
	...
	private AbstractRedisClient createRedisClient() {
		...
		if (isClusterAware()) {
			...
			ClusterTopologyRefreshOptions clusterTopologyRefreshOptions = ClusterTopologyRefreshOptions.builder()
					...
					.adaptiveRefreshTriggersTimeout(90, TimeUnit.SECONDS)
					.build();
			...
			this.clusterCommandExecutor = new ClusterCommandExecutor(
					new LettuceCustomClusterConnection.LettuceClusterTopologyProvider(clusterClient),
					new LettuceCustomClusterConnection.LettuceClusterNodeResourceProvider(clusterClient), EXCEPTION_TRANSLATION);
